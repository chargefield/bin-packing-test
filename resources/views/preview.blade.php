...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        <div style="width: 100%;">
            @foreach($bins as $bin)
            <div style="padding: 1.3rem 0 2rem;{{ !$loop->first ? ' border-top: 2px solid #ccc;': '' }}">
                <h2 style="margin-top: 0;">Order Sheet {{ $loop->iteration }} <span style="font-size: .8rem;">({{ $bin->width() }}x{{ $bin->height() }})</span></h2>
                <div style="display: flex; width: 100%">
                    <div class="bin" style="position: relative; background-color: #fff; border: 1px solid #000; width: {{ $bin->width() }}rem; height: {{ $bin->height() }}rem; overflow: hidden;">
                        ...
                        @foreach($bin->getFreeSpace() as $node)
